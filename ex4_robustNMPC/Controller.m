...
classdef Controller 
    ...
    properties
        ...
        linear  %bool flag indicating if controller is linear or non-linear
    end
    ...
    methods
        ...
        function [u, out, info] = solve(obj, x, vars, verbose)
            ...
            if obj.linear
                % for linear problems use YALMIP
                if ~isempty(obj.prob)
                ...
                % for non-linear problems use CasADi
                if ~isempty(obj.prob)
                    % set verbosity level
